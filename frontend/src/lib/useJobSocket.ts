...
export function useJobSocket(jobId: string): UseJobSocketReturn {
  ...
  const loadInitialStatus = useCallback(async () => {
    try {
      ...
      // For jobs in progress, don't set progress - rely on WebSocket for real-time progress
      setJobState(prev => ({
        ...prev,
        status: STATUS_LABELS[jobStatus.status] || jobStatus.status,
        progress: 0, // Start at 0, WebSocket will update this
        isConnected: true
      }));
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (data.type === 'status' && data.status) {
            ...
            setJobState(prev => ({
              ...prev,
              progress: (data.progress || 0) * 100 // Convert to percentage for display
            }));
          }
        } catch (error) {
