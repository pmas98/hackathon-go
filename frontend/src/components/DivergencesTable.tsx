...
export function DivergencesTable({ errors, filters, className }: DivergencesTableProps) {
  ...
  const paginatedErrors = sortedErrors.slice(startIndex, endIndex);
    ...
    <div className={cn("space-y-6", className)}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0 p-4 bg-white/5 rounded-xl border border-white/10">
        ...
        <div className="text-sm text-purple-200">
          ...
          <span className="text-white font-semibold">{Math.min(endIndex, sortedErrors.length)}</span> de{' '}
          <span className="text-white font-semibold">{sortedErrors.length}</span> divergências
        </div>
